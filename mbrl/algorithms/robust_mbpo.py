...
ROBUST_MBPO_LOG_FORMAT = mbrl.constants.EVAL_LOG_FORMAT + [
    ("epoch", "E", "int"),
...
def rollout_model_and_populate_sac_buffer(
    ...
    for i in range(rollout_horizon):
        ...
        model_env.dynamics_model.set_propagation_method(
            propagation_method=None)
        # pred_next_obs, pred_rewards, pred_dones, \
        # _ = model_env.dynamics_model.forward(
        #     action, use_propagation=False)
        pred_next_obs, pred_rewards, pred_dones, _ = model_env.step(
            action, sample=False)
        model_env._current_obs = obs
        q1s = torch.zeros(pred_next_obs.shape[0],
                          pred_next_obs.shape[1], 1)
        q2s = torch.zeros(pred_next_obs.shape[0],
                          pred_next_obs.shape[1], 1)
        for i in range(pred_next_obs.shape[0]): #for each ensemble
        # member
            with torch.no_grad():
                q1, q2 = agent.critic(
                   torch.tensor(pred_next_obs[i]).to(agent.device),
                   torch.tensor(action).to(agent.device)
                )
            q1s[i] = q1.cpu().data
            q2s[i] = q2.cpu().data
            del q1, q2
        worst_indices_1 = torch.argmin(q1s, dim=0)
        worst_indices_2 = torch.argmin(q2s, dim=0)
        model_env.dynamics_model.set_worst(
                                            worst_indices_1,
                                            # worst_indices_2
            )
        model_env.dynamics_model.set_propagation_method(
            propagation_method="worst_model")
